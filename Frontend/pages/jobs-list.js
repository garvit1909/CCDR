...
export default function JobGrid() {
    ...
    // useEffect(() => {
    //     fetch('http://localhost:3000/api/jobbing').then((a) => {
    //         return a.json();
    //     })
    //         .then((parsed) => {
    //             setBlog(parsed)
                            ...
                            <div className="banner-hero banner-single banner-single-bg">
                                <div className="block-banner text-center">
                                    <h3 className="wow animate__animated animate__fadeInUp">
                                        <span className="color-brand-2">Marketing & Sales</span> 
                                    </h3>
              ...
              <span><StarButton/></span>
                ...
                <div className="right-info">
                  <Link legacyBehavior href="company-details">
                    <a className="name-job">{blogitem.ids}</a>
                  </Link>
                 
                  <span className="small">
                  <i className="fas fa-globe" style={{ marginRight: '5px', fontSize: '14px' }}></i>
{blogitem.domain}</span>
                </div>
              </div>
              <div className="card-block-info">
                <h6>
                ...
                <div className="mt-5">
                  {/* <span className="card-briefcase">Fulltime</span> */}
                  <span className="card-time">
                    <span> Dated : {blogitem.date}</span>
                  </span>
                </div>
                <p className="font-sm color-text-paragraph mt-15"> {truncateText(blogitem.problem, 15 )}</p>
                <div className="mt-30">
  {blogitem.tags && blogitem.tags.split(',').map((tag, index) => (
    <Link legacyBehavior href="/jobs-grid" key={index}>
      <a className="btn btn-grey-small mr-5">{tag.trim()}</a>
    </Link>
  ))}
</div>
